...
function fromObject$1(ProtoClass, obj) {
...
function omitUndefinedAndEmptyLists$1(obj) {
...
function getEnumString$1(Enum, enumNumber) {
  let upperCase = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
...
function getEnumNumber$1(Enum, enumString) {
...
function serialize$1(ProtoClass, obj, converter) {
  if (obj === undefined) return undefined;
  const convertedObj = converter ? converter(obj) : obj;
  return fromByteArray(fromObject$1(ProtoClass, convertedObj).serializeBinary());
}
function deserialize$1(ProtoClass, base64String, converter) {
...
function optimizeFilters(filters) {
  ...
  return [...groupingMap.entries()].map(_ref2 => {
    let [, groupedFilters] = _ref2;
    const {
      type,
      context,
      precision,
    ...
    const query = groupedFilters.flatMap(_ref3 => {
      let {
        q
      } = _ref3;
